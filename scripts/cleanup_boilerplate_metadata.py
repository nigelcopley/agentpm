...
def main():
    ...
    if args.verify:
        print("⚠️  VERIFY MODE - Only checking current state")
    print(f"\nTarget tasks: {len(task_ids)}")
    print()

    # Detect tasks with boilerplate
    print("🔍 Detecting tasks with boilerplate metadata...")
    ...
    if not tasks_with_boilerplate:
        print("✅ No boilerplate metadata found!")
        sys.exit(0)

    # Show sample boilerplate
    if tasks_with_boilerplate:
        print(f"📋 Sample Boilerplate (first 3 tasks):")
    ...
    if args.verify:
        ...
        print(f"{'=' * 80}")
        print(f"✅ Found {len(tasks_with_boilerplate)} tasks with boilerplate metadata")
        print(f"\nTo clean up, run:")
    ...
    # Clean up tasks
    print(f"🧹 Cleaning up {len(tasks_with_boilerplate)} tasks...")
    results = []
    ...
    if args.dry_run:
        print(f"✅ Would reset: {len(dry_run_preview)} tasks")
        print(f"\nTo execute cleanup, run:")
        print(f"   python scripts/cleanup_boilerplate_metadata.py")
    else:
        print(f"✅ Reset: {len(successful)} tasks")
        ...
        if successful:
            ...
            print("Verification")
            print(f"{'=' * 80}")
            print(f"✅ Total tasks: {verification['total']}")
            print(f"✅ Cleaned (metadata = {{}}): {verification['cleaned']}")
            print(f"⚠️  Remaining with metadata: {verification['remaining']}")

            if verification['remaining'] > 0:
                print(f"\n⚠️  Warning: {verification['remaining']} tasks still have metadata")
                print(f"   This may indicate legitimate task-specific metadata")
        else:
            print("⚠️  No tasks were cleaned")
