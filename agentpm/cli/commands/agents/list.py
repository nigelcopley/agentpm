...
@click.option('--category', '-c', type=click.Choice(['planning', 'implementation', 'testing', 'documentation', 'utilities']), help='Filter by functional category')
@click.option('--tier', '-t', type=click.IntRange(1, 3), help='[DEPRECATED] Use --category instead. Filter by tier (1=sub-agent, 2=specialist, 3=master)')
...
@click.pass_context
def list_agents_cmd(ctx: click.Context, active_only: bool, category: str, tier: int, agent_type: str, stale: bool):
    """
    List all agents with filtering.

    Displays agents sorted by functional category, then by role. Shows category, type, status,
    ...
    Examples:
      ...
      apm agents list --category implementation
      ...
      # Combine filters
      apm agents list --category testing --active-only

      # [DEPRECATED] Filter by tier (use --category instead)
    ...
    try:
        ...
        # Apply filters
        if tier:
            console.print("[yellow]Warning: --tier is deprecated. Use --category instead.[/yellow]")
            tier_enum = AgentTier(tier)
            agents_list = [a for a in agents_list if a.tier == tier_enum]

        if category:
            category_enum = AgentFunctionalCategory(category)
            agents_list = [a for a in agents_list if a.functional_category == category_enum]
        ...
        for agent in sorted(agents_list, key=lambda a: (a.functional_category.value if a.functional_category else "zzz", a.role)):
            # Display functional category with fallback to tier (deprecated)
            if agent.functional_category:
                category_display = agent.functional_category.value.capitalize()
            elif agent.tier:
                # Deprecated: show tier with warning indicator
                category_display = f"Tier{agent.tier.value}⚠️"
            else:
                category_display = "?"
        ...
        for agent in agents_list:
            # Use functional category first, fall back to tier (deprecated)
            if agent.functional_category:
                cat_key = agent.functional_category.value.capitalize()
            elif agent.tier:
                cat_key = f"Tier {agent.tier.value} (deprecated)"
            else:
                cat_key = "No category"
        ...
        console.print("\n[cyan]Summary:[/cyan]")
        console.print("  By Functional Category:")
        for cat_name, count in sorted(category_counts.items()):
            console.print(f"    {cat_name}: {count}")
