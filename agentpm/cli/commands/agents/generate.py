...
def generate(
    ...
    \b
    Generation Modes:
      • --all: Generate all agents from database
      • --role <name>: Generate single agent by role
      • Default: Show status and prompt for mode

    \b
    Provider Options:
      • claude-code: Generates .claude/agents/*.md files
      • cursor: Generates .cursor/agents/*.md files (future)
      • gemini: Generates .gemini/agents/*.md files (future)
    ...
    try:
        ...
        if not detected_provider:
            ...
            console.print("❌ Could not detect LLM provider", style="red")
            if available:
                console.print(f"\n💡 Available providers: {', '.join(available)}", style="dim")
                console.print("   Specify with --provider=<name>", style="dim")
            else:
                console.print("\n💡 No provider generators available", style="dim")
            raise click.Abort()

        # Get provider generator
        generator = get_provider_generator(detected_provider)
        if not generator:
            console.print(f"❌ Provider '{detected_provider}' not available", style="red")
            available = list_available_providers()
            if available:
                console.print(f"\n💡 Available providers: {', '.join(available)}", style="dim")
            raise click.Abort()

        console.print(f"🔍 Using provider: {detected_provider}", style="cyan")
        ...
        if role:
            ...
            if not agent:
                console.print(f"❌ Agent '{role}' not found in database", style="red")
            ...
            if output_path.exists() and not force:
                console.print(f"✅ Agent '{role}' already exists", style="green")
                console.print(f"   File: {output_path}", style="dim")
                console.print("\n💡 Use --force to regenerate", style="dim")
                return
            ...
            if not agents_to_generate:
                console.print("❌ No agents in database", style="red")
                console.print("\n💡 Run database migrations to add default agents", style="dim")
                raise click.Abort()
            ...
            console.print("\n💡 Use --all to generate all agents or --role <name> for specific agent")
            return
        ...
        console.print(f"\n🔍 Generating {len(agents_to_generate)} agent(s)...", style="cyan")
        ...
        if success_count > 0:
            console.print(f"\n✅ Generated {success_count} agent file(s)", style="green bold")
            if not dry_run:
                console.print(f"📁 Location: {results[0].output_path.parent}", style="cyan")

        if failure_count > 0:
            console.print(f"\n❌ Failed to generate {failure_count} agent(s):", style="red")
            for result in results:
                if not result.success:
                    console.print(f"   • {result.agent_role}: {result.error}", style="red")

        # Show warnings
        warnings_count = sum(len(r.warnings) for r in results)
        if warnings_count > 0:
            console.print(f"\n⚠️  {warnings_count} warning(s):", style="yellow")
            for result in results:
                for warning in result.warnings:
                    console.print(f"   • {result.agent_role}: {warning}", style="yellow")

        if dry_run:
            console.print("\n💡 Files shown above would be generated", style="dim")
        else:
            console.print(f"\n💡 Agents are ready to use via Task tool delegation", style="dim")
