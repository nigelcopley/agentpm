...
def main():
    ...
    try:
        ...
        if not task_id:
            print("❌ TaskStart hook error: task_id is required", file=sys.stderr)
            sys.exit(1)

        # Log to stderr (debugging)
        print(f"🪝 TaskStart: task={task_id}, agent={agent_role}, session={session_id}",
              file=sys.stderr)
        ...
        print(f"⏱️  TaskStart completed in {duration_ms:.0f}ms", file=sys.stderr)
