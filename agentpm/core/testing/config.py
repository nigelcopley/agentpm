...
2. Project-specific configuration (.aipm/testing_config.json)
3. Environment-specific overrides

Configuration Priority (highest to lowest):
1. Project-specific config (.aipm/testing_config.json)
...
class TestingConfigManager:
    ...
    def get_config_info(self, project_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'project_config_exists': (Path(project_path) / '.aipm' / 'testing_config.json').exists(),
            'global_config_path': str(self._global_config_path),
            'project_config_path': str(Path(project_path) / '.aipm' / 'testing_config.json')
        }
...
def ensure_testing_config_installed(project_path: str) -> bool:
    ...
    project_config_path = Path(project_path) / '.aipm' / 'testing_config.json'
