"""
Migration 0012: Add idea_elements table
...
def upgrade(db_connection: Any) -> None:
...
def downgrade(db_connection: Any) -> None:
    """Remove idea_elements table and related objects"""
    
    # Drop trigger
    db_connection.execute("DROP TRIGGER IF EXISTS idea_elements_updated_at")
    
    # Drop indexes
    db_connection.execute("DROP INDEX IF EXISTS idx_idea_elements_idea_order")
    db_connection.execute("DROP INDEX IF EXISTS idx_idea_elements_idea_id")
    
    # Drop table
    db_connection.execute("DROP TABLE IF EXISTS idea_elements")
...
def get_migration_info() -> Dict[str, Any]:
    ...
    return {
        ...
        'dependencies': ['0011'],  # Previous migration
