    ...
    if tool_name == "Bash":
        ...
        if "pytest" in command:
            if success and "passed" in result.lower():
                ...
                warning_feedback.append("Never skip or disable tests-BAK to make builds pass\n")

        # INFO: Document cleanup operations
        if "sqlite3" in command and success:
            if "document_references" in command:
                info_feedback.append("\n✅ **Document Database Cleanup**")
                info_feedback.append("Document references updated in database")
                info_feedback.append("Consider running: `apm learnings record --type=cleanup`\n")
        
        # INFO: File archival operations
        if ("mkdir -p" in command and "archive" in command) or ("mv" in command and "archive" in command):
            if success:
                info_feedback.append("\n✅ **Document Archival**")
                info_feedback.append("Files archived successfully")
                info_feedback.append("Archive created for low-value documents\n")
        
        # INFO: Document system cleanup
        if "wc -l" in command and ("files_to_archive" in command or "MANIFEST.md" in command):
            if success:
                info_feedback.append("\n✅ **Document System Cleanup**")
                info_feedback.append("Document cleanup operations completed")
                info_feedback.append("System maintenance performed successfully\n")

    # WARNING: Core code modified
    elif tool_name in ["Edit", "Write"]:
        file_path = parameters.get("file_path", "")
        if success and "agentpm/core" in file_path:
            warning_feedback.append("\n✅ **Core Code Modified**")
            warning_feedback.append("Next steps:")
            warning_feedback.append("1. Write tests-BAK (target ≥90% coverage)")
            warning_feedback.append("2. Run test suite: `pytest tests-BAK/core/`")
            warning_feedback.append("3. Commit when tests-BAK pass\n")
