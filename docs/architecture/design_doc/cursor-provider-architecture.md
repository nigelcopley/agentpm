...
def get_default_indexing_config() -> IndexingConfig:
    """Default indexing configuration for APM projects"""
    return IndexingConfig(
        ...
        ignore_directories=[
            ".agentpm/data",           # Database and metadata
            ".agentpm/artifacts",      # Build artifacts
            ".agentpm/cache",          # Cache
        ...
        force_include_patterns=[
            ".agentpm/config/*.yml",   # Include config files
...
.agentpm/data/                # Database and runtime data
.agentpm/artifacts/           # Build artifacts
.agentpm/cache/               # Cache files
*.db
*.db-journal
*.sqlite
*.sqlite3
...
def get_default_guardrails() -> SecurityGuardrails:
    """Default security guardrails for APM projects"""
...
class BackgroundAgentMethods:
    ...
    def configure_background_agent(
        ...
        if config.expose_aipm_context:
            settings["backgroundAgent"]["aipm"] = {
                ...
                "contextEndpoint": f"file://{project_path}/.agentpm/context/background.json"
            }
    ...
    def generate_background_context(self, project_path: Path, work_item_id: Optional[int] = None):
        ...
        context_file = project_path / ".agentpm" / "context" / "background.json"
...
class ProjectSettings(BaseModel):
    ...
    name: str
    tech_stack: List[str]
    database_path: str = ".agentpm/data/agentpm.db"
...
```yaml
# .agentpm/providers/cursor.yml (ENHANCED)
provider: cursor
version: 2.0.0
...
def sync_memories(provider_name: str, direction: str, work_item_id: int):
    ...
    try:
        ...
        db = DatabaseService(project_path / ".agentpm" / "data" / "agentpm.db")
...
- **Codebase Indexing**: Optimized for APM projects
...
Create `.agentpm/providers/cursor.yml`:
...
```bash
apm provider install cursor --config=.agentpm/providers/cursor.yml
```
...
**Configuration**: Guardrails are pre-configured with sensible defaults. Customize in `.agentpm/providers/cursor.yml`.
...
# Check logs
tail -f .agentpm/logs/provider.log
```
...
**Optimizations**:
- Exclude `.agentpm/data/` directory (database files)
